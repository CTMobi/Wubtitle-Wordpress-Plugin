...
class ApiStoreRephrase {
	...
	public function get_rephrase( $request ) {
		...
		if ( isset( $params['attachmentId'] ) && ! empty( $params['attachmentId'] ) && ! empty( get_post( $params['attachmentId'] ) ) ) {
			return $this->add_post_rephrase( $rephrase, $params['attachmentId'] );
		}
		// External Video.
		if ( isset( $params['videoId'] ) && ! empty( $params['videoId'] ) ) {
			return $this->add_post_rephrase( $rephrase, $params['videoId'] );
		}
	...
	public function add_post_rephrase( $rephrase, $id_video ) {
		...
		$trascript_post_elements = array(
			'post_title'   => $rephrase_info->title,
		...
		$trascript_post_id = wp_insert_post( $trascript_post_elements );
